...
import { requestList, requestDetail, requestUpdate, requestCreate, requestDelete, requestUsersRoleIDList, requestSetRole } from '@/api/app/sys/user'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const data = { user_ids: [] }
      data.user_ids.push(row.id)
      this.$confirm('是否确定删除?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        requestDelete(data).then(() => {
          this.$message({
            message: '删除成功',
            type: 'success'
          })
    ...
    handleSetRole(row) {
      ...
      requestUsersRoleIDList(this.temp.id).then(response => {
